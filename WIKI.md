...
> **最后更新**: 2026-05-07 (v2.15.4: AiHubMix API Key 全量轮换 —— 旧 key `sk-LEc0Cx...0e`（`agent-review-service/.env::AI_API_KEY`，评审用）与 `sk-OyTz84ZS...8a`（Java 主服务在线生成用）经 `curl /v1/chat/completions` 验证均返回 401 `this key is not enabled`，即 AiHubmix 网关侧已禁用，是导致 v2.15.3 中 795 条假投票的根因；新 key `sk-u9ycJy...Fd` 实测 `/v1/models` 返回 200 且 `gpt-5-mini` 实际推理 200，已统一替换 4 处：`agent-review-service/.env::AI_API_KEY`、`src/main/resources/application.yml::ai.api-key`、`scripts/model_manage.py::AIHUBMIX_KEY`、`scripts/verify_and_fix.py::AIHUBMIX_KEY`；后续重启 Java 主服务和 `agent-review-service` 后即可对清理脚本回退的 795 个 `ready` 状态对战重跑评审)  
> **v2.15.3** (2026-05-07): 新增 `scripts/cleanup_failed_reviews.py` 清理脚本 —— 用于清除 `agent-review-service` 因 LLM 调用失败（如 401 `this key is not enabled` / `quota exhausted`）触发 `dimension_agent._fallback_score` 兜底而产生的"假 tie"评审；① 通过 `votes.dim_*_reason LIKE '%LLM 评审失败，降级为 tie%'` 精确识别 6 个维度任一字段命中的污染投票；② 级联清理 `elo_history`（按 battle_id）→ 删除 `votes` → 把无残留投票的 `battles` 状态从 `voted` 回退到 `ready` 并清空 `winner`（保留 `responseA/B`，可直接重跑评审无需重新调用模型）→ 按各模型剩余 `elo_history` 最新一条回滚 `models.elo_score`，若无任何剩余历史则恢复默认 `1500.00`；③ **同步清理 Redis 缓存**：`edu_arena:battle:{id}` / `edu_arena:model:detail:{id}` / `edu_arena:leaderboard:all` / `edu_arena:leaderboard:elo_history` / `edu_arena:models:active` 全部 `DELETE`，永久计数器 `edu_arena:stats:total_votes` 用 `DECRBY` 按删除条数递减（其余短 TTL 缓存自然过期，无需处理）；④ 默认 dry-run 自动 rollback，加 `--apply` 才真实写库；支持 `--stats-only` / `--pattern` / `--battle-status-to` / `--keep-battles` / `--no-recompute-elo` / `--no-redis` 灵活参数；⑤ 实测一次清理 795 条假投票 / 1590 条 elo_history / 30 个模型 ELO 回滚 / `total_votes` 计数器 2841→2046  
